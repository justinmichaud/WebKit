//@ runDefault
// StringBuilder correctness: single-piece accumulator over a long run (V += token), base64-encode shape, one site.
...
function ref(n) {
    var a = [];
    for (var i = 0; i < n; i++) { a.push(tab[i & 63]); }
    return a.join("");
}
noInline(ref);
function check(msg, x, y) { if (x !== y) throw new Error("single-long: " + msg + " got " + JSON.stringify(x) + " want " + JSON.stringify(y)); }

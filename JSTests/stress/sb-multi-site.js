//@ runDefault
// StringBuilder correctness: multiple accumulation sites on the same local (V += a; V += b; V += c) -- gated off.
...
function ref(n) {
    var a = [];
    for (var i = 0; i < n; i++) { a.push("a" + i, "b" + (i*2), "c" + (i & 3)); }
    return a.join("");
}
noInline(ref);
function check(msg, x, y) { if (x !== y) throw new Error("multi-site: " + msg + " got " + JSON.stringify(x) + " want " + JSON.stringify(y)); }

...
// Build an SVG-path-like string via chained concat: (segments) intermediate ropes + 1 final.
// Returns the final (unresolved) rope.
function buildPath(segments, seed) {
    var s = "M" + tok(seed) + " " + tok(seed + 1);
    for (var k = 0; k < segments; k++)
        s = s + " L" + tok(seed + k * 2) + " " + tok(seed * 3 + k);
    return s;
}
noInline(buildPath);

// StringBuilder sweep microbenchmark (noregress): MANY short single-site accumulations (each well under the promote threshold).
...
function frame() {
    for (var w = 0; w < 1; w++) sink += build(6).length;
}
noInline(frame);
var t0 = preciseTime();

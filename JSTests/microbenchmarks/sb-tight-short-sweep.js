// StringBuilder sweep microbenchmark (noregress): tight loop of very-short accumulation, no GC (the inline-short-rope residual case).
...
function frame() {
    for (var w = 0; w < 1; w++) sink += build(8).length;
}
noInline(frame);
var t0 = preciseTime();
